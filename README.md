...
Крошки стоят в самом верху возрастного экрана. `Главная` и `Болталка` работают как возврат к выбору возрастной группы. Отдельной верхней кнопки `Группы` и подписи возраста на этих экранах нет.
...
- пользователь нажимает карточку, чтобы выбрать её и сразу перевернуть;
- на лицевой стороне есть заметная плашка `Перевернуть`;
...
- над плеером есть отдельная подсказка внимательно слушать и запоминать детали;
...
- исчезать заранее, до самого низа прокрутки;
